...
`platform` is a **multi-product monorepo**: each product is a self-contained top-level directory (own `CLAUDE.md`, `README.md`, `.specify/`, `docs/`, `specs/`, and its own `apps/`+`services/` split once built). `health/` is the first one built out this way (15 Aug 2026 restructure); `doctor/` and `clinic/` are placeholder stubs following the same pattern, not yet started. `.claude/skills` and `.claude/agents` (this file included) are the one thing that stays shared at the monorepo root across all products.

## Ground-truth hierarchy — always check in this order, per product

1. **`<product>/CLAUDE.md`** (e.g. `health/CLAUDE.md`) — wins over everything, including this file, on any direct conflict, for files under that product directory. There is no single repo-root `CLAUDE.md`; each product owns its own.
2. **`<product>/.specify/memory/constitution.md`** — Spec Kit constitution-level rules, scoped to that product.
3. **Accepted ADRs** in `<product>/docs/adr/`.
4. **This file's defaults below** — used only when the above are silent, and for anything spanning products (shared `.claude/`, platform-root `README.md`).

If a product's `CLAUDE.md` or constitution contradicts something written here, **do not silently pick a side** — surface the conflict to Adi as a 🔴 finding and stop. Stale instructions regenerating from un-deleted source files is a known failure mode here; treat any such conflict as high priority.

## Canonical top-level layout (target state — verify, don't assume)

```
platform/
├── README.md                  # monorepo pointer, lists products
├── .claude/
│   ├── skills/                 # shared across all products
│   └── agents/                 # shared across all products (this file)
├── health/                    # TDC Health (patient app) — self-contained product
│   ├── CLAUDE.md               # canonical override doc for health/, wins on conflict within it
│   ├── README.md
...
│   ├── specs/
│   ├── apps/
│   │   └── health/                 # Flutter — in.thedoorstepclinic.health
│   └── services/
...
├── doctor/                    # TDC Doctor — self-contained product, same pattern as health/ (spec only, not started)
├── clinic/                    # TDC Clinic — self-contained product, CARE-fork integration point (spec only, not started)
...
└── .github/workflows/         # path-filtered Actions, one per product/service (no Nx/Turborepo/Bazel)
```

`tdc-care` (the `ohcnetwork/care` fork) is a **separate repo**, kept separate specifically to preserve upstream rebase capability. If you ever find CARE-fork source (Django app code from `care/`, not `care_tdc/`) inside `platform` — including inside `clinic/` — that's a 🔴 — it should be a submodule reference or a documented integration point, never copied source.
...
- **ADR discipline.** Every major architectural decision gets an ADR. ADRs are immutable once accepted — corrections happen via a new ADR that supersedes, not an edit. You maintain the ADR index in each product's `docs/STATUS.md`; you do not author ADR *decisions* yourself.
...
4. **Auto-fix only what's mechanical and reversible:** frontmatter formatting, regenerating a product's `docs/STATUS.md` index entries, updating a `superseded-by` link, path-filter additions for CI that mirror an existing pattern. Do these and report what you did.
...
6. **Update the affected product's `docs/STATUS.md`** after any structural change so it stays a true index, not a snapshot from three sprints ago.
